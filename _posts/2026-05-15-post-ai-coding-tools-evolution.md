...
2026 年 4 月，Warp 在 GitHub 开源了其 100 万行 Rust 代码库，一个终端模拟器获得了近 6 万颗星。同月，OpenAI 为竞品 Claude Code 发布了官方插件，让 Codex 直接给 Claude Code 打下手。5 月，OpenAI 又把 Codex 直接塞进了 ChatGPT 移动端 App——手机变成了 Agent 的随身控制台。
...
我们在深入使用这些 AI 工具之后，逐渐感受到：这不仅是产品形态的变化，更是后 AI 编程时代工具演进方向的一次集中显现——编辑器与终端正在走向融合，移动端正在从”噱头”演变为刚需，多 Agent 协作也正在从社区 Hack 逐步沉淀为基础设施。不同路径的演化，最终都在指向同一个方向。

**编程工具的中心，正在从”代码编辑”转向”Agent 调度”。** IDE、终端、移动端、多 Agent 系统，看似不同方向，实际上都在围绕同一件事演化：如何让 Agent 持续工作，同时让人类保持控制。下文中——Agent 环境、移动端、多 Agent 编排——就是这个趋势的三个投影。
...
一个是 Atom 创始人的第二次尝试，一个是红杉投资的终端项目。起点不同，方向一致。
...
2025 年，Zed 做了一个关键的 UI 决策：引入面板布局切换器，在"经典模式"和"Agent 模式"之间切换。这不是加了个侧边栏——这是承认 **AI 优先的工作流是一种独立的操作模式**。Agent 可以读写代码（Unified Diff）、并行运行、内置 Skills 系统、支持计划模式。
...
Warp 的创始人 Zach Lloyd 说："它不是终端，也不是 IDE。它是智能开发环境（Agentic Development Environment, ADE）。"Warp 不是"AI 增强的终端"，它经历了四代进化：
...
**它们的终点很可能是同一个东西**——一个 Agent 原生的开发环境，同时具备代码编辑、命令执行、Agent 调度、多模型管理的能力。区别只是从编辑器还是从终端出发。

这个收敛不是偶然的。当 Agent 成为编程的主体，"编辑器"和"终端"的边界本身就失去了意义。Agent 不关心 GUI，不关心 cursor，它的核心接口是文件系统、shell、context 和 task loop。IDE 和 Terminal 最终都会退化为 Agent Runtime 的不同外壳。无论你从哪个入口进来，最终都需要一个让 Agent 高效工作、让人保持控制的环境。
...
更深层的变化在于：过去的开发模式是人即 CPU，每一步都需要实时操作。Agent 编程把人从执行中解放出来——但解放之后，人需要的是一个异步的监督接口，而不是另一个实时操作界面。移动端恰好是这个接口。
...
Codex 移动端的意义在于它**定义了移动端 Agent 控制的标准形态**——不是把开发环境搬到手机上，不是通过第三方中继间接控制，而是 Agent 原生内置的移动控制能力。之前这只能通过社区方案实现，现在官方直接给了答案。
...
社区发展出三种路径，图中展示了各自的特点和差异。此外，移动端还可以通过 [Claude Code Router](https://github.com/musistudio/claude-code-router) 按任务类型切换模型。
...
这种"各有所长"不是偶然的，而是 LLM 能力结构的内在差异——推理能力强的模型不一定执行效率高，代码能力强的模型不一定文档质量好，上下文理解深的模型不一定响应速度快。这种"能力单极化"让单 Agent 在复杂任务中必然遇到瓶颈，多 Agent 协作本质上是模型专业化之后的工程必然。
...
2026 年 3 月，OpenAI 发布了一个值得关注的项目：[Codex Plugin for Claude Code](https://github.com/openai/codex-plugin-cc)。它让 Claude Code 可以直接调用 Codex 完成三种任务：
...
目前每个控制平面都在做自己的 Agent 接入和调度逻辑——PixCode 用自己的 CLI 封装，ZCode 用自己的 IDE 集成，Paseo 用自己的编排 Skills。但这不会持续太久，因为 Agent 生态的标准化协议栈已经成型。

三层协议正在各就各位，分别解决 Agent 生态中不同层面的互操作问题：

| 协议 | 解决的问题 | 来源 |
|------|-----------|------|
| **MCP**（Model Context Protocol） | Agent 与工具/数据源的连接——Agent 怎么"动手" | Anthropic |
| **A2A**（Agent-to-Agent Protocol） | Agent 与 Agent 之间的发现、通信与协作——Agent 之间怎么"对话" | Google，捐给 Linux Foundation，50+ 企业支持 |
| **ACP**（Agent Client Protocol） | 编辑器/IDE 与 AI Agent 的标准接口——任何编辑器都能无缝使用任何 Agent | 社区标准，JetBrains、Zed 等已支持 |

这三层恰好对应了全文讨论的三个维度：MCP 让 Agent 能操作外部工具，ACP 让任何编辑器/终端都能成为 Agent 的界面（呼应第一章"编辑器与终端收敛"），A2A 让多个 Agent 能自主协作（呼应本章"多 Agent 编排"）。当这些协议被控制平面广泛采纳，开发者只需要选择调度策略，而不需要关心 Agent 的接入方式。开放的态度已经确立（OpenAI 给 Claude Code 写插件就是信号），标准也已就位，接下来是工具层对齐协议的过程。
...
三条线——Agent 环境（编辑器/终端收敛）、移动控制（从噱头到必备）、多 Agent 编排（从选配到标配）——互相咬合，形成闭环。

更准确地说，软件开发正在经历一次根本性的位置转移：从 **Human-in-the-loop**（人类在流程内部，每一步都需要亲自执行）到 **Human-on-the-loop**（人类在流程上方，定义目标、审核结果、处理异常）。编辑器、终端、移动端、控制平面，都是为这个新位置服务的工具——不是让人更高效地写代码，而是让人更高效地管理写代码的 Agent。
