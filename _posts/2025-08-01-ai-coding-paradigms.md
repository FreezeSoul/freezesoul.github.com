...
如果说 Spec-Driven 是"先写完整规格再动手"，那么 Plan Mode 就是"先想清楚怎么做再动手"——它比 Spec 轻量，比 YOLO 有规划。
...
**什么时候用**：
- 任务有一定复杂度，但不需要写完整规格文档
- 你大概知道要怎么做，但不确定细节
- 想保持一定的控制权，但又不想被规格束缚

**和 Spec 的区别**：

| 方面 | Spec-Driven | Plan Mode |
|------|-------------|-----------|
| 文档要求 | 完整的规格文档 | 简化的执行计划 |
| 粒度 | 细致到字段和接口 | 粗粒度到步骤 |
| 灵活性 | 低，按文档执行 | 高，可调整计划 |
| 适用场景 | 企业项目、生产环境 | 快速迭代、个人项目 |
...
把上面五种范式放在一起看，你会发现一个有趣的现象：**它们的核心诉求都是"规范化AI开发，提高可控性"**。
...
graph LR
    ...
    subgraph 五种范式的应对策略
        ...
        B2[Plan: 计划先行<br/>分步执行+动态调整]
        B3[Spec: 前置约束<br/>用规格文档定义边界]
        B4[YOLO: 事后审查<br/>快速出结果再人工审核]
        B5[Solo: 人工主导<br/>开发者保持完全控制]
    end
    ...
    style B4 fill:#ff6b6b,stroke:#d32f2f,color:#b71c1c
    style B5 fill:#48dbfb,stroke:#00b8b0,color:#008b82
...
| 方面 | 效率优先 | 控制优先 |
|------|----------|----------|
| 开发速度 | Vibe/YOLO更快 | Spec/Plan/Solo更慢 |
| 代码质量 | 质量不稳定 | 质量相对可控 |
| 维护成本 | 后期成本高 | 前期投入大 |
| 适用场景 | 原型、验证 | 生产、长期 |
...
在所有范式之中，Spec-Driven 和 Plan Mode 代表了一个有趣的谱系——它们都强调"先想再做"，但重和轻的程度不同。

### 为什么需要 Plan？

```mermaid
graph TD
    subgraph AI编程的效率-控制谱系
        A[完全失控<br/>YOLO] --> B[轻度规划<br/>Vibe]
        B --> C[中度规划<br/>Plan]
        C --> D[重度规划<br/>Spec]
        D --> E[完全掌控<br/>Solo]
    end
    ...
    style D fill:#1dd1a1,stroke:#00cc7f,color:#009966
    style E fill:#48dbfb,stroke:#00b8b0,color:#008b82
...
| 场景 | 推荐范式 | 理由 |
|------|----------|------|
| 快速验证想法 | Vibe | 追求速度，不需要规划 |
| 日常功能开发 | Plan | 需要规划但不需要完整规格 |
| 企业级系统 | Spec | 质量要求高，需要完整规范 |
| 一次性脚本 | YOLO | 能用就行，不用维护 |
| 核心业务代码 | Solo | 质量要求极高 |
...
# 不好的计划（太细）
1.1 创建 User 类，包含 name, email, password 字段
1.2 email 需要唯一性验证
1.3 password 最小长度 8 位
...
```
...
1. 创建用户模型
   ✓ User 类存在
   ✓ 包含 name, email, password 字段

2. 实现注册接口
   ✓ POST /register 返回 200
   ✓ 用户数据写入数据库
```
...
graph TD
    ...
    style F fill:#ff6b6b,stroke:#d32f2f,color:#b71c1c
    style G fill:#48dbfb,stroke:#00b8b0,color:#008b82

```

### 组合使用策略

**"Vibe探索，Plan开发，Spec生产"**：
...
graph TD
    ...
    style E fill:#feca57,stroke:#fbc02d,color:#f57f17
    style F fill:#ff6b6b,stroke:#d32f2f,color:#b71c1c
...
| 常规功能开发 | Plan Mode | 平衡效率和质量 |
| 内部工具 | 简化规范 + 人工审查 | 效率优先，质量要求相对宽松 |
| 实验性功能 | Vibe验证 + Plan重构 | 快速探索，成功后再规范化 |
...
**现实是**：很多团队在使用混合方案——用 Markdown 写规范/计划，用现有 AI 工具生成代码，用 Git 管理版本。这种"轻量级 Spec-driven"或"适度 Plan"在实践中也很常见。

## 总结

AI编程的各种范式，本质上都是在回答同一个问题：**如何在享受AI效率的同时保持控制？**

从 Vibe 的"差不多就行"到 Plan 的"先想清楚"，从 Spec 的"先说清楚"到 YOLO 的"让它跑"，再到 Solo 的"我来负责"——每种范式都在探索人和 AI 之间控制权的最优分配。
...
Spec Kit 等工具的出现，标志着这个转变的开始。未来可能会出现更多工具，帮助开发者更好地编写、管理和复用规范。
