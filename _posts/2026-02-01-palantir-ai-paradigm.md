...
graph TB
    subgraph "第一阶段：建立语义层"
        direction TB
        A1[数据库表/字段] --> A2["映射业务概念<br/>customer_id → 下单客户"]
        A2 --> A3["✓ 数据有含义"]
    end

    subgraph "第二阶段：构建本体论"
        direction TB
        B1["识别核心对象<br/>Customer, Order, Product"] --> B2["定义关系与动作<br/>links, actions, rules"]
        B2 --> B3["<b>业务的'词汇表'</b><br/>✓ 业务模型显性化"]
    end

    subgraph "第三阶段：引入 Skills"
        direction TB
        C1["<b>业务的'句子'</b><br/>调用本体论词汇"] --> C2["组装完整流程<br/>order-processing SKILL"]
        C2 --> C3["✓ 模块化可复用"]
    end

    subgraph "第四阶段：Agent 智能化"
        direction TB
        D1["自然语言交互<br/"审批订单""] --> D2["AI 理解并执行<br/>基于本体论"]
        D2 --> D3["✓ 业务人员可用"]
    end
