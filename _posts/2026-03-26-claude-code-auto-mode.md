...
开启也很简单，在启动时加上 flag 即可：

```bash
claude --auto-mode
```

进入会话后正常下指令就行，分类器在后台自动接管审批决策。
...
但不得不说，Codex 这种全自动模式有一种独特的魅力——它更像是在开盲盒。你扔进去一个需求，然后等它跑完，打开一看：有时候代码结构之优雅、边界处理之周全，会让你真切感受到顶级模型的实力上限在哪里。那种"它居然想到了这个"的惊艳感，是逐步审批模式里很难体会到的。即便最终交付率只有七八成，它给人的感受是：**未来的软件开发，大概就是这个样子**。
...
在启动时，务必养成限制轮次的习惯。例如使用 --max-turns 5，防止它在死循环里反复横跳。
...
从 Codex 的 bypass 到 Claude Code 的 Auto Mode，只是体现了AI CODING工具的两种不同的侧面。这个差异没有优劣，只是在不同场景下找最优解的探索。
