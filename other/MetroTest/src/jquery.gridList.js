...
(function (factory) {
  ...
  DraggableGridList.prototype = {
    ...
    draggableDefaults: {
      ...
      resizeContainer:'.grid-container',
    ...
    * */
    ...
    {
        this.gridList.createItem(item);
        /*
        * Connect to this.Items
        * */
        this.items = this.gridList.items;
        this._createGridSnapshot();
    ...
    * */
    _bindResizeEvents:function(){
        this._onResizeMouseMove = this._bindMethod(this._onResizeMouseMove);
        this._onResizeMouseUp = this._bindMethod(this._onResizeMouseUp);
        this._onResizeMouseDown = this._bindMethod(this._onResizeMouseDown);
        $(this.draggableOptions.resizeHandle).bind('mousedown',this._onResizeMouseDown);
    },
    /*
    * 取消绑定拖拽缩放事件
    * */
    _unbindResizeEvents:function(){
        $(this.draggableOptions.resizeHandle).unbind('mousedown',this._onResizeMouseDown);
    },
    /*
    * 鼠标点击初始化缩放操作
    * */
    _onResizeMouseDown:function(e){
        ...
        this._currentResizeElement = $(e.currentTarget).closest('li');
        this._startX =  e.clientX;
        this._startY =  e.clientY;
        this._startW =  this._currentResizeElement.width();
        this._startH =  this._currentResizeElement.height();
        $(this.draggableOptions.resizeContainer).bind('mousemove',this._onResizeMouseMove);
        $(this.draggableOptions.resizeContainer).bind('mouseup',this._onResizeMouseUp);
    },
    /*
    * 鼠标移动触发缩放
    * */
    _onResizeMouseMove:function(e){
        event.preventDefault();
        var width = this._startW + e.clientX - this._startX;
        var height = this._startH + e.clientY - this._startY;
        this._resizingW = parseInt(width/this._cellWidth) +1;
        this._resizingH = parseInt(height/this._cellHeight) +1;
        this.resizeItem(this._currentResizeElement, [this._resizingW,this._resizingH]);
        this._currentResizeElement.addClass('resizeDrag');
        this._currentResizeElement.css({
            width: width,
            height: height
        });
        this._highlightPositionForItem(this._getItemByElement(this._currentResizeElement));
        if(this.draggableOptions.onResize)
            this.draggableOptions.onResize(this._currentResizeElement);
     },
    /*
    * 鼠标弹起触发关闭缩放操作
    * */
    _onResizeMouseUp:function(){
        event.preventDefault();
        $(this.draggableOptions.resizeContainer).unbind('mousemove',this._onResizeMouseMove);
        $(this.draggableOptions.resizeContainer).unbind('mouseup',this._onResizeMouseUp);
        this._currentResizeElement.removeClass('resizeDrag');
        this.resizeItem(this._currentResizeElement, [this._resizingW,this._resizingH]);
    ...
    * */
    ...
    _snapItemPositionToGrid: function(item) {
      ...
      * 支持在最右端定位
      * */
      if(col > this._maxGridCols - item.w)
        col = col + item.w;
      return [col, row];
    },
